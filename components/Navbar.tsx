...
export default async function Navbar() {
    ...
    return (
        <div className="sticky top-0 z-999 w-full bg-(--bg) border-b border-white/30 p-2 flex items-center justify-between inset-0">
            <div className="flex items-center gap-16">
                <div className="text-2xl font-serif ml-10">Art<span className="text-amber-500">hub</span></div>
                <ul className="flex gap-20">
                    {
                        navOptions.map((op, index) => (
                            <li key={index} className="hover-col text-white">
            ...
            <div className="flex gap-40 items-center">
                <div className="relative max-w-80">
                    <input placeholder="Search" className="w-full rounded-2xl p-2 pl-8 pr-7 bg-white/13 focus:outline-0 text-sm" />
                    <Search size={18} className="absolute left-2 top-1/2 -translate-y-1/2 text-white/80 hover-col" />
                </div>
